...
\begin{figure}[htbp]
  ...
  \end{tikzpicture}
  \caption{Families of MCDM methods.}
...
goal, criteria, and alternatives, illustrated in its complete form in \Cref{fig:ahp-hierarchy};
comparative judgement elicits the relative importance of
elements at the same hierarchical level through pairwise comparison; and synthesis of priorities
aggregates these judgements into an overall priority vector.

\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[
      font=\footnotesize,
      lvl/.style={draw, rounded corners=2pt, align=center, minimum width=26mm,
                  minimum height=9mm},
      goal/.style={lvl, fill=gray!12, font=\footnotesize\bfseries},
      lk/.style={-{Latex[length=1.6mm]}, gray!55, thin}
    ]
    \node[goal] (goal) at (5.2,4.2) {Goal};
    \node[lvl] (c1) at (0,2.1)   {Criterion 1};
    \node[lvl] (c2) at (3.47,2.1) {Criterion 2};
    \node[lvl] (c3) at (6.93,2.1) {$\cdots$};
    \node[lvl] (c4) at (10.4,2.1) {Criterion $n$};
    \node[lvl] (a1) at (0,0)   {Alternative 1};
    \node[lvl] (a2) at (3.47,0) {Alternative 2};
    \node[lvl] (a3) at (6.93,0) {$\cdots$};
    \node[lvl] (a4) at (10.4,0) {Alternative $m$};
    \foreach \c in {c1,c2,c3,c4}{\draw[lk] (goal) -- (\c.north);}
    \foreach \c in {c1,c2,c3,c4}{\foreach \a in {a1,a2,a3,a4}{\draw[lk] (\c.south) -- (\a.north);}}
  \end{tikzpicture}
  \caption{The complete three-level AHP hierarchy.}
  \label{fig:ahp-hierarchy}
  \floatsource{\emph{Source:} adapted from \textcite{sostar_assessment_2023}.}
\end{figure}

Comparisons are expressed on a
...
relationships exist, such as those between battery capacity and weight. Unlike the complete
hierarchy of \Cref{fig:ahp-hierarchy}, AHP is used only to weight
