...
Use this skill when Codex needs to work on Word, Excel, or PowerPoint files through the external `officecli` binary. This is a lightweight adapter: do not clone, copy, or vendor the OfficeCLI source repository into this project.
...
- The task creates, reads, edits, validates, or visually checks `.docx`, `.xlsx`, or `.pptx`.
- The user mentions Word, Excel, PowerPoint, Office, DOCX, XLSX, PPTX, slides, workbook, spreadsheet, deck, rendered preview, screenshot, or formatting QA.
...
- The primary deliverable is LaTeX, Markdown, Python code, PDF paper parsing, or a web app.
- A spreadsheet task needs heavy pandas/openpyxl work; use the existing `xlsx` workflow and OfficeCLI only for Office-native checks.
- A slide task needs the existing `pptx` design workflow; OfficeCLI can still help with preview, DOM edits, and validation.
