...
> **Bảng này là NGUỒN routing chuẩn của dự án — phải khớp danh sách skill/subagent thực tế.** Khi thêm/bớt skill hoặc agent, cập nhật bảng này (xem "Bảo trì routing" cuối skill).

**Skills** — chạy trong hội thoại chính (đọc `.claude/skills/<ten>/SKILL.md` trước khi làm):

| Loại việc | Skill |
|---|---|
| Viết VN/EN, LaTeX, citation, tổng hợp lit review | `professional-writing` |
| Tính AHP, TOPSIS, sensitivity | `mcdm-toolkit` |
| Phân tích khảo sát Likert | `likert-analysis` |
| Thao tác dữ liệu lẻ, làm sạch nhanh | `data-pipeline` |
...
| Bối cảnh hoá toolkit khi sang project mới | `project-switch` |

**Subagents** — gọi qua Agent tool, chạy phòng riêng (ưu tiên khi output dài dòng / tự chứa / lặp cùng loại thợ):

| Loại việc | Subagent |
|---|---|
| Đọc sâu nhiều paper, trích tiêu chí/citation | `literature-researcher` |
| Crawl/làm mới dữ liệu laptop khối lượng lớn | `data-gatherer` |
| Tạo → viết chuẩn → audit một skill | `skill-smith` |
| Xử lý Office files dài/nhiều file cần render/validate/QA riêng | `office-docs` |
...
## Bảo trì routing (BẮT BUỘC)

task-processor là nhạc trưởng — nó chỉ điều phối đúng nếu bảng routing ở Bước 4 khớp thực tế. Vì vậy **mỗi khi thêm / bớt / gộp / đổi tên một skill hoặc subagent, phải cập nhật đồng bộ 3 nơi:**

1. **Bảng routing Bước 4 ở chính skill này** — thêm/xoá dòng, phân đúng loại Skill hay Subagent.
2. `AGENTS.md` mục 5 (bảng routing + bảng subagent).
3. `.claude/skills/project-switch/upgrade-log.md` (đã đổi gì · vì sao · bài học).

Chưa cập nhật đủ 3 nơi thì coi như việc thêm/bớt skill **chưa hoàn tất**. Skill `skill-smith` và `project-switch` phải nhắc/thực hiện bước này ở phần kết.
